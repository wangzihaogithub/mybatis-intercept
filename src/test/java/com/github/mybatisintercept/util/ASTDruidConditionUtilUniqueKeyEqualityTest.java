...
public class ASTDruidConditionUtilUniqueKeyEqualityTest {

    public static void main(String[] args) {
        ASTDruidConditionUtilUniqueKeyEqualityTest test = new ASTDruidConditionUtilUniqueKeyEqualityTest();

        test.selectJoinUniqueKeyEquality();
        test.select();
        test.mysql8Cte();
        test.update();
        test.delete();
    ...
    public void mysql8Cte() {
        ...
        String cte1 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("with user_cte(dept_id,name) as (select dept_id,name from user ) SELECT t1.id, t2.* " +
                "from dept t1 left join user_cte t2 on t1.id = t2.dept_id", "tenant_id = 2", table2);
        ...
        String cte2 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey(
                "WITH user_cte (dept_id, name) AS (SELECT dept_id, name FROM user)," +
                        ...
                        "LEFT JOIN user_cte2 t3 ON t1.id = t3.dept_id", "tenant_id = 2", table2);
        Assert.assertEquals("WITH user_cte (dept_id, name) AS (\n" +
                "\t\tSELECT dept_id, name\n" +
                "\t\tFROM user\n" +
                "\t\tWHERE user.tenant_id = 2\n" +
    ...
    @Test
    public void selectJoinUniqueKeyEquality() {
        Map<String, List<String>> table2 = new HashMap<>();
        table2.put("dept", Arrays.asList("id"));
        table2.put("user", Arrays.asList("id"));
        String injectConditionN2 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("select t1.a, t2.b from user t1 ,dept t2 where t1.dept_id = t2.id and t1.id = 2 and t1.id = ? ",
                "tenant_id = 2 and name like 'a' or b > 10", table2);
        Assert.assertEquals("select t1.a, t2.b from user t1 ,dept t2 where t1.dept_id = t2.id and t1.id = 2 and t1.id = ? ", injectConditionN2);

        Map<String, List<String>> table = new HashMap<>();
        table.put("dept", Arrays.asList("id"));
        table.put("user", Arrays.asList("id"));
        String injectConditionN = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("select t1.a, t2.b from user t1 left join dept t2 on t1.dept_id = t2.id where t1.id = ?",
                "tenant_id = 2 and name like 'a' or b > 10", table);
        Assert.assertEquals("select t1.a, t2.b from user t1 left join dept t2 on t1.dept_id = t2.id where t1.id = ?", injectConditionN);


        Map<String, List<String>> table1 = new HashMap<>();
        table1.put("dept", Arrays.asList("id"));
        table1.put("user", Arrays.asList("id"));
        String injectConditionN1 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("select t1.a, t2.b from user t1 left join dept t2 on t1.dept_id = t2.id ",
                "tenant_id = 2 and name like 'a' or b > 10", table1);
        Assert.assertEquals("SELECT t1.a, t2.b\n" +
                "FROM user t1\n" +
                "\tLEFT JOIN dept t2 ON t1.dept_id = t2.id\n" +
                "WHERE t1.tenant_id = 2\n" +
                "\tAND t1.name LIKE 'a'\n" +
                "\tOR t1.b > 10", injectConditionN1);
    ...
    public void select() {
        ...
        String self = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey(" SELECT\n" +
                        "            a.id,\n" +
                        "            a.NAME,\n" +
        ...
        String from = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("SELECT\n" +
                "\ta.id,\n" +
                "\ta.oid,\n" +
                "\ta.rid,\n" +
                "\ta.type,\n" +
                ...
                "\tAND a.rid = r.id \n" +
                "\tAND a.is_delete = 0 \n" +
                "\tAND r.is_delete = 0", "tenant_id = 2", table2);
        ...
        String wherePart = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("SELECT t1.a, t2.b\n" +
                ...
                "WHERE t1.id = ?  and t1.type = 1", "tenant_id = 1", table2);
        Assert.assertEquals("SELECT t1.a, t2.b\n" +
                "FROM user t1\n" +
                "\tLEFT JOIN dept t2\n" +
                "\tON t1.dept_id = t2.id\n" +
        ...
        String where = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("SELECT t1.a, t2.b\n" +
                "FROM user t1\n" +
                "\tLEFT JOIN dept t2\n" +
                "\tON t1.dept_id = t2.id\n" +
                "\t\tAND t2.tenant_id = 2 and t2.type = 1\n" +
                "WHERE t1.id = ?  and t1.type = 1\n" +
                "\tAND t1.tenant_id = 2", "tenant_id = 1", table2);
        Assert.assertEquals("SELECT t1.a, t2.b\n" +
                "FROM user t1\n" +
                "\tLEFT JOIN dept t2\n" +
                "\tON t1.dept_id = t2.id\n" +
        ...
        String insert3 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("SELECT * INTO p_user_1 FROM p_user  ", "tenant_id = 2", table2);
        Assert.assertEquals("SELECT *\n" +
                "INTO p_user_1\n" +
                "FROM p_user\n" +
                "WHERE p_user.tenant_id = 2", insert3);

        String set = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("set global query_cache_type = OFF ", "tenant_id = 2", table2);
        Assert.assertEquals("set global query_cache_type = OFF ", set);

        String at = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("SELECT @@auto_generate_certs ", "tenant_id = 2", table2);
        Assert.assertEquals("SELECT @@auto_generate_certs ", at);

        String show = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("SHOW VARIABLES ", "tenant_id = 2", table2);
        Assert.assertEquals("SHOW VARIABLES ", show);

        String dual = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("SELECT 1 FROM DUAL ", "tenant_id = 2", table2);
        Assert.assertEquals("SELECT 1 FROM DUAL ", dual);

        // join 1个表
        String join1 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("select t1.a, t2.b from user t1 left join dept t2 on t1.dept_id = t2.id where t1.id = ?", "tenant_id = 2", table2);
        Assert.assertEquals("select t1.a, t2.b from user t1 left join dept t2 on t1.dept_id = t2.id where t1.id = ?", join1);
        ...
        Assert.assertEquals("select t1.a, t2.b from user t1 right join dept t2 on t1.dept_id = t2.id where t1.id = ?", join2);
        ...
        Assert.assertEquals("select t1.a, t2.b from user t1  join dept t2 on t1.dept_id = t2.id where t1.id = ?", join3);

        // join 2个表
        String join4 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("select t1.a, t2.b from user t1 left join dept t2 on t1.dept_id = t2.id  left join dept t3 on t1.dept_id = t3.id  where t1.id = ?", "tenant_id = 2", table2);
        Assert.assertEquals("select t1.a, t2.b from user t1 left join dept t2 on t1.dept_id = t2.id  left join dept t3 on t1.dept_id = t3.id  where t1.id = ?", join4);
        ...
        Assert.assertEquals("select t1.a, t2.b from user t1 right join dept t2 on t1.dept_id = t2.id right join dept t3 on t1.dept_id = t3.id where t1.id = ?", join5);
        ...
        Assert.assertEquals("select t1.a, t2.b from user t1 right join dept t2 on t1.dept_id = t2.id left join dept t3 on t1.dept_id = t3.id where t1.id = ?", join6);
        ...
        Assert.assertEquals("select t1.a, t2.b from user t1 left join dept t2 on t1.dept_id = t2.id right join dept t3 on t1.dept_id = t3.id where t1.id = ?", join7);
        ...
        Assert.assertEquals("select t1.a, t2.b from user t1  join dept t2 on t1.dept_id = t2.id  join dept t3 on t1.dept_id = t3.id where t1.id = ?", join8);

        String select1 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("select * from user  where id = ?", "tenant_id = 2", table2);
        Assert.assertEquals("select * from user  where id = ?", select1);

        String select223 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("select * from user  where 1 = ?", "tenant_id = 2", table2);
        Assert.assertEquals("SELECT *\n" +
                "FROM user\n" +
                "WHERE 1 = ?\n" +
                "\tAND user.tenant_id = 2", select223);


        String select222 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("select * from user  where 1 = 1", "tenant_id = 2", table2);
        Assert.assertEquals("SELECT *\n" +
                "FROM user\n" +
                "WHERE 1 = 1\n" +
                "\tAND user.tenant_id = 2", select222);

        String select2 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("select * from user", "tenant_id = 2", table2);
        Assert.assertEquals("SELECT *\n" +
                "FROM user\n" +
                "WHERE user.tenant_id = 2", select2);

        // union 联合
        String select3 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("select id,name from user t1 union select id,name from user", "tenant_id = 2", table2);
        ...
        // 子查询
        String select4 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("select * from (select id,name from user t1 union select id,name from user) t ", "tenant_id = 2", table2);
        Assert.assertEquals("SELECT *\n" +
                "FROM (\n" +
                "\tSELECT id, name\n" +
                "\tFROM user t1\n" +
        ...
        String select5 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("select * from (select id,name from user t1 union all select id,name from user) t1 left join (select * from (select id,name from user t1 union all select id,name from user) t2) on t1.id = t2.id", "tenant_id = 2", table2);
        Assert.assertEquals("SELECT *\n" +
                "FROM (\n" +
                "\tSELECT id, name\n" +
                "\tFROM user t1\n" +
        ...
        String select6 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey(
                "select * from " +
                        ...
                        "left join user t3 on t1.id = t3.id", "tenant_id = 2", table2);
        Assert.assertEquals("SELECT *\n" +
                "FROM (\n" +
                "\tSELECT id, name\n" +
                "\tFROM user t1_1\n" +
                ...
                "\t) t2\n" +
                "\tON t1.id = t2.id\n" +
                "\tLEFT JOIN user t3 ON t1.id = t3.id", select6);

        String select61 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey(
                "select * from " +
                        "(select id,name from user t1_1 union all select id,name from user t1_2) t1 " +
                        "left join (select * from (select id,name from user t2_1_1 union all select id,name from user t2_1_2) t2_1) t2 on t1.id = t2.id " +
                        "left join user t3 on t1.id = t3.id " +
                        "where id = 1 ", "tenant_id = 2", table2);
        Assert.assertEquals("SELECT *\n" +
                "FROM (\n" +
                "\tSELECT id, name\n" +
                "\tFROM user t1_1\n" +
        ...
        String select7 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("select * from user left join dept on dept.id = user.dept_id", "tenant_id = 2", table2);
        Assert.assertEquals("SELECT *\n" +
                "FROM user\n" +
                "\tLEFT JOIN dept ON dept.id = user.dept_id\n" +
                "WHERE user.tenant_id = 2", select7);

        String select8 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("select * from user t1 left join dept on dept.id = t1.dept_id", "tenant_id = 2", table2);
        Assert.assertEquals("SELECT *\n" +
                "FROM user t1\n" +
                "\tLEFT JOIN dept ON dept.id = t1.dept_id\n" +
                "WHERE t1.tenant_id = 2", select8);

        String select9 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("select * from user  left join dept t1 on t1.id = user.dept_id", "tenant_id = 2", table2);
        Assert.assertEquals("SELECT *\n" +
                "FROM user\n" +
                "\tLEFT JOIN dept t1 ON t1.id = user.dept_id\n" +
                "WHERE user.tenant_id = 2", select9);

        // or
        String select10 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("select * from user  where id = 1 or name = '1' ", "tenant_id = 2", table2);
        Assert.assertEquals("SELECT *\n" +
                "FROM user\n" +
                "WHERE (id = 1\n" +
                "\t\tOR name = '1')\n" +
        ...
        String select11 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("select * from user  where (id = 1 or name = '1') and (id = 2 or name = '2') ", "tenant_id = 2", table2);
        Assert.assertEquals("SELECT *\n" +
                "FROM user\n" +
                "WHERE (id = 1\n" +
                "\t\tOR name = '1')\n" +
        ...
        String select12 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("select * from user  where (id = 1 and name = '1') or (id = 2 and name = '2') ", "tenant_id = 2", table2);
        Assert.assertEquals("SELECT *\n" +
                "FROM user\n" +
                "WHERE ((id = 1\n" +
                "\t\t\tAND name = '1')\n" +
        ...
        // EXISTS
        String select13 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("select * from user  where (id = 1 and name = '1') or (id = 2 and name = '2')  and EXISTS (select id from p where p.id = id)", "tenant_id = 2", table2);
        Assert.assertEquals("SELECT *\n" +
                "FROM user\n" +
                "WHERE ((id = 1\n" +
                "\t\t\tAND name = '1')\n" +
        ...
        String select14 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("select * from user  where (id = 1 and name = '1') or (id = 2 and name = '2')  and EXISTS (select id from p1 left join p2 on p2.id = p1.dept_id where p1.id = user.id)", "tenant_id = 2", table2);
        Assert.assertEquals("SELECT *\n" +
                "FROM user\n" +
                "WHERE ((id = 1\n" +
                "\t\t\tAND name = '1')\n" +
        ...
        String select15 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("select case when EXISTS (select id from p1 left join p2 on p2.id = p1.dept_id where p1.id = user.id) then 1 end from user  where (id = 1 and name = '1') or (id = 2 and name = '2')  " +
                "and EXISTS (select id from p1 left join p2 on p2.id = p1.dept_id where p1.id = user.id)", "tenant_id = 2", table2);
        ...
        String select16 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("select (select count(1) from p3 where p3.id = user.id) from user  where (id = 1 and name = '1') or (id = 2 and name = '2')  " +
                "and EXISTS (select id from p1 left join p2 on p2.id = p1.dept_id where p1.id = user.id)", "tenant_id = 2", table2);
        Assert.assertEquals("SELECT (\n" +
                "\t\tSELECT count(1)\n" +
                "\t\tFROM p3\n" +
        ...
        // case WHEN END
        String casewhen = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("SELECT\n" +
        ...
        // @
        String select17 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("SELECT\n" +
                "* \n" +
                "FROM\n" +
                "\t(\n" +
                "\tSELECT\n" +
    ...
    public void update() {
        ...
        String updateexist = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey(" UPDATE user  SET `status` = 2 WHERE id =1 and tenant_id = 1", "tenant_id = 2", table2);
        Assert.assertEquals(" UPDATE user  SET `status` = 2 WHERE id =1 and tenant_id = 1", updateexist);

        String join = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey(" update pipeline\n" +
                ...
                "      set pipeline.last_interview_time = t2.interview_time where pipeline.id = 1", "tenant_id = 2", table2);
        ...
        String update1 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey(" UPDATE user t1 SET `status` = 0 WHERE id = 1 or name = 2", "tenant_id = 2", table2);
        ...
        String update2 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey(" UPDATE user t1, dept t2  SET t1.`status` = t2.status WHERE t1.dept_id = t2.id", "tenant_id = 2", table2);
        ...
        String update3 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey(" UPDATE user t1, dept t2  SET t1.`status` = t2.status WHERE t1.dept_id = t2.id and t1.id in (select id from user)", "tenant_id = 2", table2);
        Assert.assertEquals("UPDATE user t1, dept t2\n" +
                "SET t1.`status` = t2.status\n" +
                "WHERE t1.dept_id = t2.id\n" +
                "\tAND t1.id IN (\n" +
    ...
    public void insert() {
        ...
        String insert1 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("insert into `base_area` (`id`, `name`) select id,name from copy ", "tenant_id = 2", table2);
        Assert.assertEquals("INSERT INTO `base_area` (`id`, `name`)\n" +
                "SELECT id, name\n" +
                "FROM copy\n" +
                "WHERE copy.tenant_id = 2", insert1);

        String insert2 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("replace into `base_area` (`id`, `name`) select id,name from copy ", "tenant_id = 2", table2);
    ...
    public void delete() {
        ...
        String delete1 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey("delete from user", "tenant_id = 2", table2);
        Assert.assertEquals("DELETE FROM user\n" +
                "WHERE user.tenant_id = 2", delete1);

        String delete2 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey(" delete from user t1  WHERE id = 1 or name = 2", "tenant_id = 2", table2);
        Assert.assertEquals("DELETE FROM user t1\n" +
                "WHERE (id = 1\n" +
                "\t\tOR name = 2)\n" +
                "\tAND t1.tenant_id = 2", delete2);

        String delete3 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey(" delete from user t1, dept t2 WHERE t1.dept_id = t2.id", "tenant_id = 2", table2);
        Assert.assertEquals("DELETE FROM user t1, dept t2\n" +
                "WHERE t1.dept_id = t2.id\n" +
                "\tAND t1.tenant_id = 2\n" +
                "\tAND t2.tenant_id = 2", delete3);

        String delete4 = ASTDruidTestUtil.addAndConditionIgnoreUniqueKey(" delete from user t1, dept t2 WHERE t1.dept_id = t2.id and t1.id in (select id from user)", "tenant_id = 2", table2);
        Assert.assertEquals("DELETE FROM user t1, dept t2\n" +
                "WHERE t1.dept_id = t2.id\n" +
                "\tAND t1.id IN (\n" +
                "\t\tSELECT id\n" +
